...
<Modal {open} title={t('invoices.markAsPaid')} onclose={() => { showDatePicker = false; onclose(); }}>
  {#snippet children()}
    <div class="flex flex-col gap-3">
      ...
      >
        <div>
          <div class="font-medium text-success-800">{t('invoices.paidToday')}</div>
      ...
      {#if !showDatePicker}
        ...
        >
          {t('invoices.chooseOtherDate')}
        </button>
      {:else}
        <div class="flex items-end gap-2">
          <div class="flex-1">
            <label class="block text-xs font-medium text-gray-500 mb-1">{t('invoices.paymentReceivedDate')}</label>
            <input
              type="date"
              bind:value={paidDate}
          ...
          <Button onclick={() => markPaid(paidDate)} loading={saving}>{t('common.confirm')}</Button>
        </div>
      {/if}
    </div>
  {/snippet}
