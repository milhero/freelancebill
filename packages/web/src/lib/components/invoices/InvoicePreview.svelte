<script lang="ts">
  ...
  function handleScroll() {
    if (wrapperEl) {
      hasScrolled = wrapperEl.scrollTop > 10;
    }
  }
...
<div class="relative" bind:this={containerEl}>
  <div
    bind:this={wrapperEl}
    class="rounded-xl border border-gray-200 dark:border-gray-200 overflow-hidden bg-white dark:bg-gray-100"
    style="height: {pdfHeight || 700}px; overflow-y: auto;"
  ...
  >
    {#if pdfUrl}
      ...
      <div style="height: {pdfHeight}px; background: white;">
    ...
    {:else if loading}
      <div class="flex items-center justify-center h-full bg-white dark:bg-gray-100">
        <div class="w-4 h-4 border-2 border-gray-200 border-t-indigo-500 rounded-full animate-spin"></div>
      </div>
    {:else}
      <div class="flex flex-col items-center justify-center h-full text-gray-300 gap-2 bg-white dark:bg-gray-100">
  ...
  <!-- Reset pill -->
  {#if hasScrolled && pdfUrl}
    <button
      onclick={resetScroll}
      class="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/60 backdrop-blur-sm text-white text-[11px] font-medium shadow-lg hover:bg-black/80 transition-all duration-200 animate-fade-in"
    >
